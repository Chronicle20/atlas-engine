...
function friendlyItemDrop(eim, mob) {
    if (mob.getId() == 9300061) {
        ...
        mob.getMap().broadcastMessage(CWvsContext.serverNotice(6, "The Moon Bunny made rice cake number " + cakes + "."));
    }
}
...
function friendlyDamaged(eim, mob) {
    if (mob.getId() == 9300061) {
        ...
        if (bunnyDamage > 5) {
            const CWvsContext = Java.type('connection.packets.CWvsContext');
            broadcastMessage(CWvsContext.serverNotice(6, "The Moon Bunny is feeling sick. Please protect it so it can make delicious rice cakes."));

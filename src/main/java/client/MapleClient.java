        ...
        if (party.isPresent()) {
            ...
            if (party.get()
                ...
                for (MaplePartyCharacter pchr : party.get()
                        .getMembers()) {
                    if (pchr != null && pchr.getId() != idz && (lchr == null || lchr.getLevel() <= pchr.getLevel()) && map.getCharacterById(pchr.getId())
                            .isPresent()) {
                        lchr = pchr;
                    }
                }
                ...
                if (!(channel == -1 || shutdown)) {
                    if (!cashshop) {
                        if (!this.serverTransition) { // meaning not changing channels
                            ...
                            if (bl != null) {
                                wserv.loggedOff(player.getId(), channel, player.getBuddylist()
                                        .getBuddyIds());
                            }
                        }
                    } else {

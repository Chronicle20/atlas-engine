...
public enum AutobanFactory {
   ...
   public void alert(MapleCharacter chr, String reason) {
      if (YamlConfig.config.server.USE_AUTOBAN == true) {
         ...
         Server.getInstance().broadcastGMMessage((chr != null ? chr.getWorld() : 0), CWvsContext.sendYellowTip(
               (chr != null ? MapleCharacter.makeMapleReadable(chr.getName()) : "") + " caused " + this.name() + " " + reason));
      }
      if (YamlConfig.config.server.USE_AUTOBAN_LOG) {
         FilePrinter.print(FilePrinter.AUTOBAN_WARNING,
               (chr != null ? MapleCharacter.makeMapleReadable(chr.getName()) : "") + " caused " + this.name() + " " + reason);
      }
   }

...
public class Equip extends Item {
   ...
   private boolean isUpgradeable, isElemental;    // timeless or reverse, or any equip that could levelup on GMS for all effects
   ...
   private void getUnitStatUpgrade(List<Pair<StatUpgrade, Integer>> stats, StatUpgrade name, int curStat, boolean isAttribute) {
      ...
      int maxUpgrade =
            randomizeStatUpgrade((int) (1 + (curStat / (getStatModifier(isAttribute) * (isNotWeaponAffinity(name) ? 2.7 : 1)))));
      if (maxUpgrade == 0) {
         return;
      }
   ...
   private void gainLevel(MapleClient c) {
      ...
      String showStr =
            "#e'" + ItemInformationProvider.getInstance().getName(this.getItemId()) + "'#b is now #elevel #r" + itemLevel + "#k#b!";
   ...
   public synchronized void gainItemExp(MapleClient c, int gain) {  // Ronan's Equip Exp gain method
      ...
      int equipMaxLevel =
            Math.min(30, Math.max(ii.getEquipLevel(this.getItemId(), true), YamlConfig.config.server.USE_EQUIPMNT_LVLUP));

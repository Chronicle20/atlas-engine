      ...
      if (c.tryacquireClient()) {
         try {
            ...
            try {
               ...
               if (response != 0) {
                  chr.sendPacket(CStoreBankDlg.fredrickMessage(response));
                  return;
               }
               ...
               if (deleteFredrickItems(chr.getId())) {
                  ...
                  for (Pair<Item, MapleInventoryType> it : items) {
                     ...
                     FilePrinter.print(FilePrinter.FREDRICK + chr.getName() + ".txt",
                           chr.getName() + " gained " + item.getQuantity() + " " + itemName + " (" + item.getItemId() + ")");
                  }

                  chr.sendPacket(CStoreBankDlg.fredrickMessage((byte) 0x1E));

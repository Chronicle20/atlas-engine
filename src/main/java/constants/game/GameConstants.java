...
public class GameConstants {
   ...
   public static final MapleDisease[] CPQ_DISEASES =
         {MapleDisease.SLOW, MapleDisease.SEDUCE, MapleDisease.STUN, MapleDisease.POISON, MapleDisease.SEAL, MapleDisease.DARKNESS, MapleDisease.WEAKEN, MapleDisease.CURSE};
   ...
   public static boolean isAranSkills(final int skill) {
      return Aran.FULL_SWING == skill || Aran.OVER_SWING == skill || Aran.COMBO_TEMPEST == skill || Aran.COMBO_FENRIR == skill
            || Aran.COMBO_DRAIN == skill || Aran.HIDDEN_FULL_DOUBLE == skill || Aran.HIDDEN_FULL_TRIPLE == skill
            || Aran.HIDDEN_OVER_DOUBLE == skill || Aran.HIDDEN_OVER_TRIPLE == skill || Aran.COMBO_SMASH == skill
            || Aran.DOUBLE_SWING == skill || Aran.TRIPLE_SWING == skill;
   }
   ...
   public static boolean isMerchantLocked(MapleMap map) {
      if (FieldLimit.CANNOTMIGRATE.check(
            map.getFieldLimit())) {   // maps that cannot access cash shop cannot access merchants too (except FM rooms).
         return true;
      }
   ...
   public synchronized static Number parseNumber(String value) {
      try {
         return NumberFormat.getInstance(YamlConfig.config.server.USE_UNITPRICE_WITH_COMMA ? Locale.FRANCE : Locale.UK)
               .parse(value);
      } catch (Exception e) {
         e.printStackTrace();
         return 0.0f;

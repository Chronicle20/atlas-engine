...
public class Server {
   ...
   private final MonitoredReentrantReadWriteLock wldLock =
         new MonitoredReentrantReadWriteLock(MonitoredLockType.SERVER_WORLDS, true);
   ...
   private final MonitoredReentrantReadWriteLock lgnLock =
         new MonitoredReentrantReadWriteLock(MonitoredLockType.SERVER_LOGIN, true);
      ...
      try {
         ...
         ps = con.prepareStatement(
               "SELECT `characters`.`name`, `characters`.`level`, `characters`.`world` FROM `characters` LEFT JOIN accounts ON accounts.id = characters.accountid WHERE `characters`.`gm` < 2 AND `accounts`.`banned` = '0'"
                     + worldQuery + " ORDER BY " + (!YamlConfig.config.server.USE_WHOLE_SERVER_RANKING ? "world, " : "")
      ...
      try (Connection con = DatabaseConnection.getConnection();
         ...
         for (Pair<String, String> namePair : changedNames) {
            FilePrinter.print(FilePrinter.CHANGE_CHARACTER_NAME,
                  "Name change applied : from \"" + namePair.getLeft() + "\" to \"" + namePair.getRight() + "\" at "
   ...
   private static void applyAllWorldTransfers() {
      try (Connection con = DatabaseConnection.getConnection();
         ...
         while (rs.next()) {
            ...
            String reason = MapleCharacter.checkWorldTransferEligibility(con, characterId, oldWorld,
                  newWorld); //check if character is still eligible
            if (reason != null) {
               removedTransfers.add(nameChangeId);
               FilePrinter.print(FilePrinter.WORLD_TRANSFER,
                     "World transfer cancelled : Character ID " + characterId + " at " + Calendar.getInstance().getTime()
                           + ", Reason : " + reason);
               try (PreparedStatement delPs = con.prepareStatement("DELETE FROM worldtransfers WHERE id = ?")) {
                  ...
                  FilePrinter.printError(FilePrinter.WORLD_TRANSFER, e,
                        "Failed to delete world transfer for character ID " + characterId);
               }
            }
         }
         rs.beforeFirst();
         List<Pair<Integer, Pair<Integer, Integer>>> worldTransfers =
               new LinkedList<>(); //logging only <charid, <oldWorld, newWorld>>
         while (rs.next()) {
            con.setAutoCommit(false);
            int nameChangeId = rs.getInt("id");
         ...
         for (Pair<Integer, Pair<Integer, Integer>> worldTransferPair : worldTransfers) {
            ...
            FilePrinter.print(FilePrinter.WORLD_TRANSFER,
                  "World transfer applied : Character ID " + charId + " from World " + oldWorld + " to World " + newWorld + " at "
                        + Calendar.getInstance().getTime());
         }
      } catch (SQLException e) {
         e.printStackTrace();
   ...
   private void removeWorldPlayerRanking() {
      if (!YamlConfig.config.server.USE_WHOLE_SERVER_RANKING) {
         ...
         List<Pair<Integer, List<Pair<String, Integer>>>> ranking =
               updatePlayerRankingFromDB(-1 * (this.getWorldsSize() - 2));  // update ranking list
   ...
   public void init() {
      ...
      try {
         ...
         log.error("Syntax error in 'world.ini'.");
         System.exit(0);
      }
   ...
   private ChannelDependencies registerChannelDependencies() {
      FredrickProcessor fredrickProcessor = new FredrickProcessor();
      ChannelDependencies channelDependencies = new ChannelDependencies(fredrickProcessor);
   ...
   private void initializeTimelyTasks() {
      ...
      tMan.register(new DueyFredrickTask(channelDependencies.fredrickProcessor()), 60 * 60 * 1000, timeLeft);
      ...
      synchronized (guilds) {
         ...
         if (mc != null) {
            ...
            if (mgc.isPresent()) {
               ...
               FilePrinter.printError(FilePrinter.GUILD_CHAR_ERROR,
                     "Could not find " + mc.getName() + " when loading guild " + id + ".");
            }
   ...
   public void guildMessage(int guildId, Packet packet) {
      guildMessage(guildId, packet, -1);
   }

   public void guildMessage(int guildId, Packet packet, int exception) {
   ...
   public void transferWorldCharacterEntry(MapleCharacter chr,
      ...
      for (Channel ch : allChannels) {
         while (!ch.finishedShutdown()) {
            try {
               ...
               log.error("Error while shutting down: ", ie);
            }
         }
      }

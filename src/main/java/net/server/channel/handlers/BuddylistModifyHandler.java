...
public class BuddylistModifyHandler extends AbstractMaplePacketHandler {
   ...
   public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
      ...
      if (mode == 0) { // refresh
         c.announce(CWvsContext.updateBuddylist(c.getPlayer().getBuddylist().getBuddies()));
      } else if (mode == 1) { // add
         ...
         if (!buddylist.isFull()) {
            try {
               ...
               if (otherName != null) {
                  buddylist.put(new BuddylistEntry(otherName, BuddyConstants.DEFAULT_GROUP, otherCid, channel, true));
   ...
   private void addBuddy(MapleClient c, String addName, String group) {
      ...
      if (buddy.isPresent()) {
         if (!buddy.get().isVisible() && group.equals(buddy.get().getGroup())) {
            c.announce(CWvsContext.buddylistMessage(BuddylistErrorMode.ALREADY_REGISTERED_AS_FRIEND));
            return;
         }
      ...
      if (buddyList.isFull()) {
         c.announce(CWvsContext.buddylistMessage(BuddylistErrorMode.YOUR_BUDDY_LIST_IS_FULL));
         return;
      }
      ...
      try {
         ...
         if (otherChar.isPresent()) {
            ...
            charWithId = new CharacterIdNameBuddyCapacity(otherChar.get().getId(), otherChar.get().getName(),
                  otherChar.get().getBuddylist().getCapacity());
         } else {
         ...
         if (charWithId != null) {
            ...
            if (channel != -1) {
               BuddyRequestInfo requestInfo = new BuddyRequestInfo(c.getChannel(), c.getPlayer().getId(),
                     c.getPlayer().getName(), c.getPlayer().getLevel(), c.getPlayer().getJob().getId(), group);
               buddyAddResult = world.requestBuddyAdd(addName, requestInfo);
            } else {
               Connection con = DatabaseConnection.getConnection();
            ...
            if (buddyAddResult == BuddyAddResult.BUDDYLIST_FULL) {
               c.announce(CWvsContext.buddylistMessage(BuddylistErrorMode.OTHER_BUDDY_LIST_IS_FULL));
               return;
            } else {
               int displayChannel;
               displayChannel = -1;
            ...
            c.announce(CWvsContext.buddylistMessage(BuddylistErrorMode.CHARACTER_NOT_FOUND));
            return;
         }
      } catch (SQLException e) {
         e.printStackTrace();

...
public final class MTSHandler extends AbstractMaplePacketHandler {
   ...
   private static byte[] getMTS(int tab, int type, int page) {
      ...
      try {
         ...
         while (rs.next()) {
            if (rs.getInt("type") != 1) {
               ...
               items.add(new MTSItemInfo(i, rs.getInt("price"), rs.getInt("id"), rs.getInt("seller"), rs.getString("sellername"),
               ...
               items.add(
                     new MTSItemInfo(equip, rs.getInt("price"), rs.getInt("id"), rs.getInt("seller"), rs.getString("sellername"),
                           rs.getString("sell_ends")));
            }
         }
         rs.close();
         ps.close();
         ps = con.prepareStatement(
               "SELECT COUNT(*) FROM mts_items WHERE tab = ? " + (type != 0 ? "AND type = ?" : "") + "AND transfer = 0");
         ps.setInt(1, tab);
         if (type != 0) {
            ps.setInt(2, type);
         }
      ...
      return CField.sendMTS(items, tab, type, page, pages); // resniff
   }
            ...
            if (i != null && c.getPlayer().getMeso() >= 5000) {
               ...
               try {
                  ...
                  if (!i.getInventoryType().equals(MapleInventoryType.EQUIP)) {
                     Item item = i;
                     ps = con.prepareStatement(
                           "INSERT INTO mts_items (tab, type, itemid, quantity, expiration, giftFrom, seller, price, owner, sellername, sell_ends) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                     ps.setInt(1, 1);
                     ...
                     Equip equip = (Equip) i;
                     ps = con.prepareStatement(
                           "INSERT INTO mts_items (tab, type, itemid, quantity, expiration, giftFrom, seller, price, upgradeslots, level, str, dex, `int`, luk, hp, mp, watk, matk, wdef, mdef, acc, avoid, hands, speed, jump, locked, owner, sellername, sell_ends, vicious, flag, itemexp, itemlevel, ringid) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                     ps.setInt(1, 1);
                     ps.setInt(2, invType.getType());
            ...
            try {
               ...
               if (rs.next()) {
                  ...
                  if (rs.getInt("type") != 1) {
                     ...
                     ii.setPosition(
                           c.getPlayer().getInventory(ItemConstants.getInventoryType(rs.getInt("itemid"))).getNextFreeSlot());
                     i = ii.copy();
                  } else {
                     Equip equip = new Equip(rs.getInt("itemid"), (byte) rs.getInt("position"), -1);
                     equip.setOwner(rs.getString("owner"));
               ...
               if (rs.next()) {
                  ...
                  if (c.getPlayer().getCashShop().getCash(4) >= price) { //FIX
                     ...
                     for (Channel cserv : Server.getInstance().getAllChannels()) {
                        MapleCharacter victim = cserv.getPlayerStorage().getCharacterById(rs.getInt("seller")).orElse(null);
                        if (victim != null) {
   ...
   public byte[] getCart(int cid) {
            ...
            try (PreparedStatement pse = con.prepareStatement("SELECT * FROM mts_items WHERE id = ?")) {
               ...
               if (rse.next()) {
                  if (rse.getInt("type") != 1) {
                     ...
                     items.add(new MTSItemInfo(i, rse.getInt("price"), rse.getInt("id"), rse.getInt("seller"),
                           rse.getString("sellername"), rse.getString("sell_ends")));
                  } else {
                     ...
                     equip.setExpiration(rs.getLong("expiration"));
                     equip.setGiftFrom(rs.getString("giftFrom"));
                     items.add(new MTSItemInfo(equip, rse.getInt("price"), rse.getInt("id"), rse.getInt("seller"),
                           rse.getString("sellername"), rse.getString("sell_ends")));
                  }
               }
            }
      ...
      return CField.sendMTS(items, 4, 0, 0, pages);
   }
   ...
   public byte[] getMTSSearch(int tab, int type, int cOi, String search, int page) {
      ...
      try {
         con = DatabaseConnection.getConnection();
         if (type != 0) {
            ps = con.prepareStatement("SELECT * FROM mts_items WHERE tab = ? " + listaitems
                  + " AND type = ? AND transfer = 0 ORDER BY id DESC LIMIT ?, 16");
         } else {
            ps = con.prepareStatement(
                  "SELECT * FROM mts_items WHERE tab = ? " + listaitems + " AND transfer = 0 ORDER BY id DESC LIMIT ?, 16");
         }
         ps.setInt(1, tab);
         if (type != 0) {
            ps.setInt(2, type);
      ...
      return CField.sendMTS(items, tab, type, page, pages);
   }
}

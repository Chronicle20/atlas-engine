...
public final class MoveLifeHandler extends AbstractMovementPacketHandler {
   ...
   public void handlePacket(InPacket p, MapleClient c) {
      ...
      short moveid = p.readShort(); // move id
      byte dwFlag = p.readByte(); // bSomeRand | 8 * (bRushMove | 2 * (2 * nMobCtrlState));
      ...
      byte nActionAndDir = p.readByte();

      int skillData = p.readInt(); // !CMob::DoSkill(v7, (unsigned __int8)dwData, BYTE1(dwData), dwData >> 16)
      ...
      int nMultiTargetSize = p.readInt();
      ...
      int nRandTimeSize = p.readInt();
      for (int i = 0; i < nRandTimeSize; i++) {
         randTimeForAreaAttack.add(p.readInt());
      }

      p.readByte();
      p.readInt();
      int start_x = p.readInt(); // possibly x
      int start_y = p.readInt(); // possibly y
      p.readInt();
      ...
      if (mobMoveStartResult) {
         ...
         if (noSkills > 0) {
            ...
            if (!(nextUse != null && monster.canUseSkill(nextUse, false) && nextUse.getHP() >= (int) (
                  ((float) monster.getHp() / monster.getMaxHp()) * 100) && mobMp >= nextUse.getMpCon())) {
      ...
      if (nextUse != null) {
         c.sendPacket(CMob.moveMonsterResponse(objectid, moveid, mobMp, aggro, nextSkillId, nextSkillLevel));
      } else {
         c.sendPacket(CMob.moveMonsterResponse(objectid, moveid, mobMp, aggro));
      }

      final MovePath res = new MovePath();
      res.decode(p);

      res.Movement().stream().filter(m -> m.getType() == 0).map(m -> m.getPosition((short) -2)).forEach(monster::setPosition);
      res.Movement().stream().map(Elem::getBMoveAction).forEach(monster::setStance);

      map.broadcastMessage(player,
            CMob.moveMonster(objectid, false, mobMoveStartResult, nextMovementCouldBeSkill, nActionAndDir, skillData,
                  multiTargetForBall, randTimeForAreaAttack, res), serverStartPos);

   ...
   public void handlePacket(InPacket p, MapleClient c) {
      ...
      if (attack.skill == Buccaneer.ENERGY_ORB || attack.skill == ThunderBreaker.SPARK || attack.skill == Shadower.TAUNT
         ...
         chr.getMap().broadcastMessage(chr,
               CUserRemote.rangedAttack(chr, attack.skill, attack.skilllevel, attack.stance, attack.numAttackedAndDamage, 0,
                     attack.allDamage, attack.speed, attack.direction, attack.display), false);
         ...
         boolean hasShadowPartner = chr.getBuffedValue(TemporaryStatType.SHADOWPARTNER) != null;
         if (hasShadowPartner) {
            bulletCount *= 2;
         }
            ...
            if (item != null) {
               ...
               if (item.getQuantity() >= bulletCount) { //Fixes the bug where you can't use your last arrow.
                  if (type == MapleWeaponType.CLAW && ItemConstants.isThrowingStar(id) && weapon.getItemId() != 1472063) {
                     if (((id == 2070007) && chr.getLevel() < 70) || (id == 2070016 && chr.getLevel() < 50)) {
                     } else {
                        projectile = id;
                        break;
                     }
                  ...
                  } else if ((type == MapleWeaponType.BOW && bow) || (type == MapleWeaponType.CROSSBOW && cbow) || (
         ...
         boolean soulArrow = chr.getBuffedValue(TemporaryStatType.SOULARROW) != null;
         boolean shadowClaw = chr.getBuffedValue(TemporaryStatType.SHADOW_CLAW) != null;
         if (projectile != 0) {
         ...
         if (projectile != 0 || soulArrow || attack.skill == 11101004 || attack.skill == 15111007 || attack.skill == 14101006
            ...
            } else if (soulArrow || attack.skill == 3111004 || attack.skill == 3211004 || attack.skill == 11101004
                  || attack.skill == 15111007 || attack.skill == 14101006 || attack.skill == 13101005) {
               visProjectile = 0;
            }
            ...
            switch (attack.skill) {
               ...
               case 13111002: // KoC Hurricane
                  packet = CUserRemote.rangedAttack(chr, attack.skill, attack.skilllevel, attack.rangedirection,
                        attack.numAttackedAndDamage, visProjectile, attack.allDamage, attack.speed, attack.direction,
                        attack.display);
                  break;
               default:
                  packet =
                        CUserRemote.rangedAttack(chr, attack.skill, attack.skilllevel, attack.stance, attack.numAttackedAndDamage,
                              visProjectile, attack.allDamage, attack.speed, attack.direction, attack.display);
                  break;
            }
            ...
            if (chr.getSkillLevel(SkillFactory.getSkill(NightWalker.VANISH).orElseThrow()) > 0
                  && chr.getBuffedValue(TemporaryStatType.DARKSIGHT) != null && attack.numAttacked > 0
                  && chr.getBuffSource(TemporaryStatType.DARKSIGHT) != 9101004) {
               ...
               chr.cancelBuffStats(TemporaryStatType.DARKSIGHT);
                  ...
                  && chr.getBuffedValue(TemporaryStatType.WIND_WALK) != null && attack.numAttacked > 0) {

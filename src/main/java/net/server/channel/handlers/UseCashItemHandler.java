...
public final class UseCashItemHandler extends AbstractMaplePacketHandler {
   ...
   private static boolean getIncubatedItem(MapleClient c, int id) {
      ...
      final int[] ids =
            {1012070, 1302049, 1302063, 1322027, 2000004, 2000005, 2020013, 2020015, 2040307, 2040509, 2040519, 2040521, 2040533, 2040715, 2040717, 2040810, 2040811, 2070005, 2070006, 4020009,};
   ...
   private static void handleAvatarMegaphone(InPacket slea, MapleClient c, MapleCharacter player,
                                             short position, int itemId, String medal, List<String> strLines) {
      Server.getInstance().broadcastMessage(c.getWorld(),
            CWvsContext.getAvatarMega(player, medal, c.getChannel(), itemId, strLines, (slea.readByte() != 0)));
      TimerManager.getInstance()
            .schedule(() -> Server.getInstance().broadcastMessage(c.getWorld(), CWvsContext.byeAvatarMega()), 1000 * 10);
   ...
   private static void handleChalkboardUsage(MapleCharacter player, String message) {
      ...
      player.getMap().broadcastMessage(CUser.useChalkboard(player, false));
      player.sendPacket(CWvsContext.enableActions());
   }
   ...
   private static void handleOwlOfMinerva(MapleClient c, short position, int itemId, int searchItemId) {
      ...
      List<Pair<MaplePlayerShopItem, AbstractMapleMapObject>> hmsAvailable =
            c.getWorldServer().getAvailableItemBundles(searchItemId);
      if (!hmsAvailable.isEmpty()) {
         remove(c, position, itemId);
      }

      c.sendPacket(CWvsContext.owlOfMinerva(c, searchItemId, hmsAvailable));
      c.sendPacket(CWvsContext.enableActions());
   }
   ...
   private static void handlePetRename(MapleClient c, MaplePet pet, short position, int itemId, String newName) {
      ...
      c.getPlayer().getMap().broadcastMessage(c.getPlayer(), CPet.changePetName(c.getPlayer(), newName, 1), true);
      remove(c, position, itemId);
   }
   ...
   private static void handleKite(MapleClient c, int itemId, String text, short position) {
      ...
      if (GameConstants.isFreeMarketRoom(c.getPlayer().getMapId())) {
         c.sendPacket(CMessageBoxPool.sendCannotSpawnKite());
         return;
      }
      ...
      if (itemType == 504) { // vip teleport rock
         ...
         if (!vip) {
            ...
            if (itemId / 1000 >= 5041 || mapId / 100000000 == player.getMapId() / 100000000) { //check vip or same continent
               ...
               if (!FieldLimit.CANNOTVIPROCK.check(targetMap.getFieldLimit()) && (targetMap.getForcedReturnId() == 999999999
                     || mapId < 100000000)) {
            ...
            if (victim != null) {
               ...
               if (!FieldLimit.CANNOTVIPROCK.check(targetMap.getFieldLimit()) && (targetMap.getForcedReturnId() == 999999999
                     || targetMap.getId() < 100000000)) {
                  if (!victim.isGM() || victim.gmLevel()
                        <= player.gmLevel()) {   // thanks Yoboes for noticing non-GM's being unreachable through rocks
         ...
         } else if (itemId == 5060001 || itemId == 5061000 || itemId == 5061001 || itemId == 5061002
               || itemId == 5061003) { // Sealing lock
            Optional<MapleInventoryType> type = MapleInventoryType.getByType((byte) p.readInt());
            if (type.isEmpty()) {
               return;
            }

            eq = player.getInventory(type.get()).getItem((short) p.readInt());
         ...
         switch ((itemId / 1000) % 10) {
            case 1: // Megaphone
               if (player.getLevel() > 9) {
                  player.getClient().getChannelServer()
                        .broadcastPacket(CWvsContext.serverNotice(2, medal + player.getName() + " : " + p.readString()));
               } else {
            ...
            case 2: // Super megaphone
               Server.getInstance().broadcastMessage(c.getWorld(),
                     CWvsContext.serverNotice(3, c.getChannel(), medal + player.getName() + " : " + p.readString(),
                           (p.readByte() != 0)));
               break;
            case 5: // Maple TV
               ...
               if (megassenger) {
                  Server.getInstance().broadcastMessage(c.getWorld(),
                        CWvsContext.serverNotice(3, c.getChannel(), medal + player.getName() + " : " + builder, ear));
               }
            ...
            case 6: //item megaphone
               ...
               if (p.readByte() == 1) { //item
                  ...
                  item = player.getInventory(type.get()).getItem((short) p.readInt());
                  if (item == null) {
                     return;
                  }
                  // thanks Conrad for noticing that untradeable items should be allowed in megas
               }
               Server.getInstance().broadcastMessage(c.getWorld(), CWvsContext.itemMegaphone(msg, whisper, c.getChannel(), item));
               break;
         ...
         player.getMap().broadcastMessage(CField.musicChange("Jukebox/Congratulation"));
         remove(c, position, itemId);
         ...
         handleAvatarMegaphone(p, c, player, position, itemId, medal, strLines);
      } else if (itemType == 540) {
         p.readByte();
         p.readInt();
         if (itemId == 5400000) { //name change
            c.sendPacket(CWvsContext.showNameChangeCancel(player.cancelPendingNameChange()));
         } else if (itemId == 5401000) { //world transfer
            c.sendPacket(CWvsContext.showWorldTransferCancel(player.cancelPendingWorldTranfer()));
         }
         remove(c, position, itemId);
         c.sendPacket(CWvsContext.enableActions());
         ...
         if (createStatus == 0) {
            c.sendPacket(CField.sendMapleLifeError(0));   // success!
            ...
            if (createStatus == -1) {    // check name
               ...
               c.sendPacket(CField.sendMapleLifeError(-1 * createStatus));
            }
         }
         ...
         if (item == null || item.getQuantity() <= 0 || MapleKarmaManipulator.hasKarmaFlag(item) || !ii.isKarmaAble(
               item.getItemId())) {
            c.sendPacket(CWvsContext.enableActions());
            return;
         }
         ...
         remove(c, position, itemId);
         c.sendPacket(CWvsContext.enableActions());
         c.sendPacket(CField.sendHammerData(equip.getVicious()));
         ...
         c.sendPacket(CField.sendVegaScroll(0x40));

         final Equip scrolled = (Equip) ii.scrollEquipWithId(toScroll, uitem.getItemId(), false, itemId, player.isGM());
         c.sendPacket(CField.sendVegaScroll(scrolled.getLevel() > curlevel ? 0x41 : 0x43));
         ...
         TimerManager.getInstance().schedule(() -> {
            ...
            player.getMap().broadcastMessage(CUser.getScrollEffect(player.getId(), scrollResult, false, false));

...
public class MapleAlliance {
   ...
   public static Optional<MapleAlliance> createAlliance(MapleParty party, String name) {
      ...
      if (alliance.isPresent()) {
         ...
         try {
            ...
            Server.getInstance().allianceMessage(id, CWvsContext.getGuildAlliances(alliance.get(), worldid), -1,
                  -1);  // thanks Vcoc for noticing guilds from other alliances being visually stacked here due to this not being updated
         } catch (Exception e) {
            e.printStackTrace();
            return Optional.empty();
         }
   ...
   public static Optional<MapleAlliance> createAllianceOnDb(List<Integer> guilds, String name) {
      ...
      try {
         ...
         PreparedStatement ps =
               con.prepareStatement("INSERT INTO `alliance` (`name`) VALUES (?)", PreparedStatement.RETURN_GENERATED_KEYS);
   ...
   public static boolean removeGuildFromAlliance(int allianceId, int guildId, int worldId) {
      ...
      srv.allianceMessage(alliance.get().getId(), CWvsContext.allianceNotice(alliance.get().getId(), alliance.get().getNotice()),
            -1, -1);
   ...
   public void saveToDB() {
      try {
         ...
         PreparedStatement ps = con.prepareStatement(
               "UPDATE `alliance` SET capacity = ?, notice = ?, rank1 = ?, rank2 = ?, rank3 = ?, rank4 = ?, rank5 = ? WHERE id = ?");

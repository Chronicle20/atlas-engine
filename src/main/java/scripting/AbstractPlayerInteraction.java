...
public class AbstractPlayerInteraction {
   ...
   private List<Integer> convertToIntegerArray(List<Object> list) {
      List<Integer> intList =
            new ArrayList<>();      // JAVA 7 Rhino script engine. Thanks Bruno, felipepm10 for noticing a typecast issue here.
      for (Object d : list) {
      ...
      if (quantity >= 0) {
         if (ItemConstants.isPet(id)) {
            ...
            if (from != null) {
               ...
               evolved.setName(from.getName().compareTo(ItemInformationProvider.getInstance().getName(from.getItemId())) != 0 ?
                     from.getName() : ItemInformationProvider.getInstance().getName(id));
         ...
         if (ItemConstants.getInventoryType(id).equals(MapleInventoryType.EQUIP)) {
            ...
            if (item != null) {
               ...
               if (YamlConfig.config.server.USE_ENHANCED_CRAFTING == true && c.getPlayer().getCS() == true) {
                  ...
                  item =
                        ItemInformationProvider.getInstance().scrollEquipWithId(item, 2049100, true, 2049100, c.getPlayer().isGM());
               }
            }
         } else {
         ...
         if (!MapleInventoryManipulator.checkSpace(c, id, quantity, "")) {
            c.getPlayer().dropMessage(1,
                  "Your inventory is full. Please remove an item from your " + ItemConstants.getInventoryType(id).name()
                        + " inventory.");
            return null;
         }
   ...
   public void resetMap(int mapid) {
      ...
      for (MapleMapObject i : getMap(mapid).getMapObjectsInRange(c.getPlayer().getPosition(), Double.POSITIVE_INFINITY,
   ...
   public int createExpedition(MapleExpeditionType type, boolean silent, int minPlayers, int maxPlayers) {
      ...
      if (!MapleExpeditionBossLog.attemptBoss(player.getId(), channel, exped,

...
public class EventInstanceManager {
   ...
   public void applyEventPlayersSkillBuff(int skillId, int skillLv) {
      SkillFactory.getSkill(skillId)
            .map(s -> s.getEffect(Math.min(skillLv, s.getMaxLevel())))
            .ifPresent(se -> getPlayerList().forEach(se::applyTo));
   }
      ...
      event_schedule = TimerManager.getInstance().schedule(() -> {
         ...
         try {
            ...
            Logger.getLogger(EventManager.class.getName())
                  .log(Level.SEVERE, "Event '" + em.getName() + "' does not implement scheduledTimeout function.", ex);
         }
      }, time);
   }
   ...
   public void registerParty(MapleCharacter chr) {
      if (chr.isPartyLeader()) {
         chr.getParty().ifPresent(p -> registerParty(p, chr.getMap()));
      }
   }
   ...
   public void unregisterPlayer(final MapleCharacter chr) {
      try {
         ...
         Logger.getLogger(EventManager.class.getName())
               .log(Level.SEVERE, "Event '" + em.getName() + "' does not implement playerUnregistered function.", ex);
      }

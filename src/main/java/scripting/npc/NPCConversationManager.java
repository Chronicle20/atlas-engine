...
public class NPCConversationManager extends AbstractPlayerInteraction {
   ...
   public MaplePlayerNPC getPlayerNPCByScriptid(int scriptId) {
      for (MapleMapObject pnpcObj : getPlayer().getMap()
            .getMapObjectsInRange(new Point(0, 0), Double.POSITIVE_INFINITY, List.of(MapleMapObjectType.PLAYER_NPC))) {
   ...
   public void cpqLobby(int field) {
      try {
         ...
         Collection<MaplePartyCharacter> members =
               c.getPlayer().getParty().map(MapleParty::getMembers).orElse(Collections.emptyList());

...
public class MapleShop {
   ...
   private static final Logger log = LoggerFactory.getLogger(MapleShop.class);
         ...
         while (rs.next()) {
            if (ItemConstants.isRechargeable(rs.getInt("itemid"))) {
               MapleShopItem starItem =
                     new MapleShopItem((short) 1, rs.getInt("itemid"), rs.getInt("price"), rs.getInt("pitch"), (byte) 0);
   ...
   private void addItem(MapleShopItem item) {
      ...
      if (itemData.isPresent()) {
         ...
         log.warn("NPC Shop [{}] has invalid sale item [{}].", npcId, item.getItemId());
      }
   }

   public void sendShop(MapleClient c) {
      c.getPlayer().setShop(this);
      c.sendPacket(CShopDlg.getNPCShop(c, getNpcId(), items));
   }
   ...
   public void buy(MapleClient c, short slot, int itemId, short quantity) {
      ...
      if (item != null) {
         if (item.getItemId() != itemId) {
            log.debug("Wrong slot number in shop {}", id);
            return;
         }
      } else {
         return;
      ...
      if (item.getPrice() > 0) {
         ...
         if (c.getPlayer().getMeso() >= amount) {
            if (MapleInventoryManipulator.checkSpace(c, itemId, quantity, "")) {
               ...
               c.sendPacket(CShopDlg.shopTransaction((byte) 0));
            } else {
               c.sendPacket(CShopDlg.shopTransaction((byte) 3));
            }
         } else {
            c.sendPacket(CShopDlg.shopTransaction((byte) 2));
         }
   ...
   public void sell(MapleClient c, MapleInventoryType type, short slot, short quantity) {
      ...
      if (canSell(item, quantity)) {
         ...
         c.sendPacket(CShopDlg.shopTransaction((byte) 0x8));
      } else {
         c.sendPacket(CShopDlg.shopTransaction((byte) 0x5));
      }
   }
   ...
   public void recharge(MapleClient c, short slot) {
      ...
      if (item.getQuantity() < slotMax) {
         ...
         if (c.getPlayer().getMeso() >= price) {
            ...
            c.sendPacket(CShopDlg.shopTransaction((byte) 0x2));
         }
      }
   }

...
public class MapleStorageInventory {
   ...
   private void move(short sSlot, short dSlot, short slotMax) {
      ...
      } else if (target.getItemId() == source.getItemId() && !ItemConstants.isRechargeable(source.getItemId())
            && !ItemInformationProvider.getInstance().isPickupRestricted(source.getItemId()) && isSameOwner(source, target)) {

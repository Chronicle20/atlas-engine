...
public class MaplePlayerShop extends AbstractMapleMapObject {
   ...
   private void addVisitor(MapleCharacter visitor) {
      for (int i = 0; i < 3; i++) {
         if (visitors[i] == null) {
            ...
            this.broadcast(CMiniRoomBaseDlg.getPlayerShopNewVisitor(visitor, i + 1));
            owner.getMap().broadcastMessage(CUser.updatePlayerShopBox(this));
            break;
         }
      }
   }
      ...
      try {
         for (int i = 0; i < 3; i++) {
            if (visitors[i] != null && visitors[i].getId() == visitor.getId()) {
               ...
               this.broadcast(CMiniRoomBaseDlg.getPlayerShopRemoveVisitor(i + 1));
               owner.getMap().broadcastMessage(CUser.updatePlayerShopBox(this));
               return;
            }
         }
         ...
         try {
            for (int i = 0; i < 3; i++) {
               if (visitors[i] != null && visitors[i].getId() == visitor.getId()) {
                  ...
                  for (int j = i; j < 2; j++) {
                     if (visitors[j] != null) {
                        owner.sendPacket(CMiniRoomBaseDlg.getPlayerShopRemoveVisitor(j + 1));
                     }
                  ...
                  for (int j = i; j < 2; j++) {
                     if (visitors[j] != null) {
                        owner.sendPacket(CMiniRoomBaseDlg.getPlayerShopNewVisitor(visitors[j], j + 1));
                     }
                  }
   ...
   public void takeItemBack(int slot, MapleCharacter chr) {
      synchronized (items) {
         ...
         if (shopItem.isExist()) {
            ...
            removeFromSlot(slot);
            chr.sendPacket(CMiniRoomBaseDlg.getPlayerShopItemUpdate(this));
         }
      }
   }
            ...
            try {
               ...
               if (c.getPlayer().getMeso() >= price) {
                  ...
                  if (canBuy(c, newItem)) {
                     ...
                     owner.sendPacket(CMiniRoomBaseDlg.getPlayerShopOwnerUpdate(soldItem, item));
   ...
   public void broadcastRestoreToVisitors() {
      ...
      try {
         for (int i = 0; i < 3; i++) {
            if (visitors[i] != null) {
               visitors[i].sendPacket(CMiniRoomBaseDlg.getPlayerShopRemoveVisitor(i + 1));
            }
         }

         for (int i = 0; i < 3; i++) {
            if (visitors[i] != null) {
               visitors[i].sendPacket(CMiniRoomBaseDlg.getPlayerShop(this, false));
            }
         }
   ...
   public void removeVisitors() {
      ...
      try {
         try {
            for (int i = 0; i < 3; i++) {
               if (visitors[i] != null) {
                  visitors[i].sendPacket(CMiniRoomBaseDlg.shopErrorMessage(10, 1));
   ...
   public void chat(MapleClient c, String chat) {
      ...
      broadcast(CMiniRoomBaseDlg.getPlayerShopChat(c.getPlayer(), chat, s));
   }
   ...
   private void recoverChatLog() {
      synchronized (chatLog) {
         for (Pair<MapleCharacter, String> it : chatLog) {
            ...
            broadcastToVisitors(CMiniRoomBaseDlg.getPlayerShopChat(chr, it.getRight(), pos));
         }
      }
   }
   ...
   public void closeShop() {
      clearChatLog();
      removeVisitors();
      owner.getMap().broadcastMessage(CUser.removePlayerShopBox(this));
   }

   public void sendShop(MapleClient c) {
      visitorLock.lock();
      try {
         c.sendPacket(CMiniRoomBaseDlg.getPlayerShop(this, isOwner(c.getPlayer())));
      } finally {
         visitorLock.unlock();
      }
   }
   ...
   public void banPlayer(String name) {
      ...
      if (target != null) {
         target.sendPacket(CMiniRoomBaseDlg.shopErrorMessage(5, 1));
   ...
   public void sendSpawnData(MapleClient client) {
      client.sendPacket(CUser.updatePlayerShopBox(this));
   }

   @Override
   public void sendDestroyData(MapleClient client) {
      client.sendPacket(CUser.removePlayerShopBox(this));
   }

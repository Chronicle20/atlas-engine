...
import connection.packets.CFieldAriantArena;
...
public class AriantColiseum {
    ...
    public AriantColiseum(MapleMap eventMap, MapleExpedition expedition) {
        ...
        for (MapleCharacter mc : players) {
            mc.sendPacket(CFieldAriantArena.updateAriantPQRanking(score));
        }
    ...
    private void broadcastAriantScoreUpdate() {
        if (scoreDirty) {
            for (MapleCharacter chr : score.keySet()) {
                chr.sendPacket(CFieldAriantArena.updateAriantPQRanking(score));
            }
            scoreDirty = false;
        }
    }
    ...
    private void showArenaResults() {
        ...
        if (map != null) {
            map.broadcastMessage(CField.showAriantScoreBoard());
            map.killAllMonsters();

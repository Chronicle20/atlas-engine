...
public class MapleQuest {
   ...
   private static final Logger log = LoggerFactory.getLogger(MapleQuest.class);
   private static final Set<Short> exploitableQuests = new HashSet<>();
   private final static MapleDataProvider questData =
         MapleDataProviderFactory.getDataProvider(new File(System.getProperty("wzpath") + "/Quest.wz"));
   ...
   public boolean forfeit(MapleCharacter chr) {
      ...
      if (timeLimit > 0) {
         chr.sendPacket(CUserLocal.removeQuestTimeLimit(id));
      }
      MapleQuestStatus newStatus = new MapleQuestStatus(this, MapleQuestStatus.Status.NOT_STARTED);
   ...
   public boolean forceComplete(MapleCharacter chr, int npc) {
      ...
      chr.sendPacket(CUser.showSpecialEffect(ShowItemGainInChatCode.QUEST_COMPLETE)); // Quest completion

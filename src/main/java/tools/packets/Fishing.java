...
public class Fishing {
   ...
   private static boolean hitFishingTime(MapleCharacter chr, int baitLevel, double yearLikelihood, double timeLikelihood) {
      ...
      if (YamlConfig.config.server.USE_DEBUG) {
         ...
         chr.dropMessage(5,
               "Score rolls - Year: " + (0.23 * yearLikelihood) + " Time: " + (0.77 * timeLikelihood) + " Meso: " + baitLikelihood);
      }
   ...
   public static void doFishing(MapleCharacter chr, int baitLevel, double yearLikelihood, double timeLikelihood) {
         ...
         switch (rand) {
            ...
            case 2:
               ...
               if (chr.canHold(itemid)) {
                  ...
                  chr.showHint("Couldn't catch a(n) #r" + ItemInformationProvider.getInstance().getName(itemid) + "#k due to #e#b"
      ...
      chr.sendPacket(CUser.showInfo(fishingEffect));
   ...
   public static int getRandomItem() {
      ...
      int[] uncommons =
            {1000025, 1002662, 1002812, 1002850, 1002881, 1002880, 1012072, 4020009, 2043220, 2043022, 2040543, 2044420, 2040943, 2043713, 2044220, 2044120, 2040429, 2043220, 2040943}; // filler' uptoo
      int[] rares =
            {1002859, 1002553, 1002762, 1002763, 1002764, 1002765, 1002766, 1002663, 1002788, 1002949, 2049100, 2340000, 2040822, 2040822, 2040822, 2040822}; // filler' uplast
   ...
   private static void debugFishingLikelihood() {
      ...
      log.debug("Hits: {}Hits10: {} Total: {}   --  %1000: {}, +10 %1000: {}", hits, hits10, total, hits * 1000 / total,
            hits10 * 1000 / total);
   }
} 
